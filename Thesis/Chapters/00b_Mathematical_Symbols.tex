...
$\lambda$              & PAPR loss weight / trade-off parameter (PRNet) \\
$\eta$                 & Training corruption level, noise-to-signal ratio (PRNet) \\
$\theta_f$             & Encoder network trainable parameters \\
$\theta_g$             & Decoder network trainable parameters \\
$\bm{r}$               & PRNet input: one-hot encoded QPSK symbol vector \\
$\hat{\bm{r}}$         & PRNet output: reconstructed symbol scores \\
$\bm{X}_{\mathrm{enc}}$ & Encoded complex subcarrier symbols (encoder output) \\
